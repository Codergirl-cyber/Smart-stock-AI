...
export default function AgentActivity() {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        const { data } = await supabase.from('agent_tasks').select('*').order('created_at', { ascending: false }).limit(12);
        if (!mounted) return;
        setTasks(data || []);
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {tasks.map((t) => (
          <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
            <div>
              <div style={{ fontWeight: 600 }}>{t.agent_name} · {t.task_type}</div>
              <div className="caption" style={{ color: 'var(--text-muted)' }}>{t.description}</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div className="mono">{t.priority}</div>
              <div className="caption">{t.status}</div>
            </div>
          </div>
        ))}
      </div>
